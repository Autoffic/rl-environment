...
class TrafficIntersectionEnvTripleLaneGUI(gym.Env):
    ...
    def __init__(
        ...
        route_file: str = None,
        generate_new_route_files: bool = True,
        reset_if_percentage_stopped: float = 0.8,
    ) -> None:

        SUMO_FILES = pathlib.Path(__file__).parents[0].joinpath("sumo-files")
        SMALL_MAP_TRIPLE_LANE = SUMO_FILES.joinpath(
            f"small-map-{TRAFFIC_INTERSECTION_TYPE}-lane"
        )
        ...
        if route_file is None:  # generating new route file if no route file is passed
            ...
            route_file = generate_routefile(
                intersection_type=TRAFFIC_INTERSECTION_TYPE,
                number_of_time_steps=total_timesteps,
            )
        ...
        self.observation_space = gym.spaces.Box(
            low=0,
            high=1000,
            shape=(int(NUMBER_OF_LANES_TO_OBSERVE),),
            dtype=numpy.float64,
        )
        ...
        if "LIBSUMO_AS_TRACI" in os.environ and os.environ["LIBSUMO_AS_TRACI"] == "1":
            print(
                f"Using libsumo implementation as per the environment variable 'LIBSUMO_AS_TRACI':{os.environ['LIBSUMO_AS_TRACI']}"
            )
    ...
    def step(self, action):
        ...
        info["average_waiting_time"] = self.current_waiting_time / max(
            1, self.last_step_vehicle_count
        )
        info["waiting_time"] = self.current_waiting_time
        info["last_step_vehicle_count"] = self.last_step_vehicle_count
        info["vehicle_entered"] = (
            self.total_vehicle_count - self.vehicle_count_up_to_last_step
        )
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self.connection_present:
            ...
            SUMO_FILES = pathlib.Path(__file__).parents[0].joinpath("sumo-files")
            SMALL_MAP_DOUBLE_LANE = SUMO_FILES.joinpath(
                f"small-map-{TRAFFIC_INTERSECTION_TYPE}-lane"
            )
            ...
            if self.generate_new_route_files:
                ...
                if (
                    self.route_file
                    != pathlib.Path(str(SMALL_MAP_DOUBLE_LANE) + ".rou.xml")
                    and self.route_file != self.passed_route_file
                ):
                ...
                new_route_file = generate_routefile(
                    intersection_type=TRAFFIC_INTERSECTION_TYPE,
                    number_of_time_steps=self.total_timesteps,
                )
        ...
        sumo_cmd = [
            ...
            "--quit-on-end",
            "--time-to-teleport",
            "-1",  # This makes it so that the vehicles won't teleport
        ]
    ...
    def calculate_stopped_percentage(self) -> float:
        ...
        """
        present = self.vehicle_ids_between_steps
        stopped_count = sum(
            1 for veh in present if self.conn.vehicle.getSpeed(veh) < 0.1
        )  # < 0.36km/hr
        return stopped_count / max(1, len(present))
    ...
    def reward_on_decrease_in_waiting_time(self) -> float:
        ...
        vehicle_count = max(
            1, self.last_step_vehicle_count
        )  # setting a minimum value, to avoid NaNs
    ...
    def reward_on_average_emission(self) -> float:
        return (
            0.0
            if self.last_step_vehicle_count < 1
            else -numpy.mean(
                [
                    self.conn.vehicle.getCO2Emission(veh)
                    for veh in self.vehicle_ids_between_steps
                ]
            )
        )
    ...
    def reward_on_average_speed(self) -> float:
        return (
            ...
            if self.last_step_vehicle_count < 1
                ...
                [
                    self.conn.vehicle.getSpeed(veh)
                    for veh in self.vehicle_ids_between_steps
                ]
            )
        )
    ...
    def composite_reward(self) -> float:
        ...
        reward = (
            self.reward_on_decrease_in_waiting_time()
